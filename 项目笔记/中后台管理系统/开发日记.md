...
# 七、封装企业级通用 Hook + 组件
封装完成后，所有 CRUD 页面10 分钟就能写一个，是简历里「提升开发效率 60%」的核心依据。
### 目录结构
![](../Images/item2.png)

# 第三阶段：系统管理模块 + 仪表盘
本阶段基于前两阶段封装的 通用 Hook + 通用组件，完成中后台核心 RBAC 权限四大模块 + 仪表盘首页，包含：用户管理、角色管理、菜单管理、字典管理，同时补全按钮权限指令。
1. 先安装图表依赖（仪表盘使用 ECharts）
```bash
npm i echarts
```
...
3. 统一编写 API 请求文件
- src/api/system/user.ts（用户模块接口）
- src/api/system/role.ts（角色模块接口）
- src/api/system/menu.ts（菜单模块接口，扩展原有）
- src/api/system/dict.ts（字典模块接口）
...
3. 是不是 app.use(router) 才运行 router/index.ts？
不是
- import router from './router' → 这行代码才会执行 router/index.ts
只要执行导入，路由文件就会运行：创建路由实例、定义路由守卫、配置路由规则。
- app.use(router) → 只是把路由「安装」到 Vue 实例中
让路由和 Vue 应用关联起来，让路由生效、监听页面 URL，并不是运行路由文件的原因。

4. Hook 是什么？
Hook（钩子）是一种函数模式，用于将组件的逻辑（状态、方法、生命周期）提取和复用。
